...
//gcc -o projmid.o projmid.c -lsqlite3 -ljansson  -lfcgi
...
void create_json_array(Report info) {
    ...
    json_object_set_new(obj, "program", json_string(info.program));
    json_object_set_new(obj, "date", json_string(info.date));
    json_object_set_new(obj, "item_borrow", json_string(info.time_borrow));
    json_object_set_new(obj, "return_time", json_string(info.return_time));
    json_object_set_new(obj, "item", json_string(info.item));
    json_object_set_new(obj, "others", json_string(info.others));
    json_object_set_new(obj, "purpose", json_string(info.purpose));
...
void loadJSON(Report *info) {  
    ...
    const char* properties[] = {"id","name", "program", "date", "time_borrow", "return_time", "item", "others", "purpose" };
    char** infovar[] = {&info->id,&info->name,&info->program,&info->date,&info->time_borrow,&info->return_time,&info->item,&info->others,&info->purpose};
...
int main(void) {
  ...
  while (FCGI_Accept() >= 0) {
   ...
   loadJSON(&info);
    ...
    // bitwise operator
    dbres |= sqlite3_bind_text(stmt, 3, info.program, -1, SQLITE_TRANSIENT); 
    dbres |= sqlite3_bind_text(stmt, 4, info.date, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 5, info.time_borrow, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 6, info.return_time, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 7, info.item, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 8, info.others, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 9, info.purpose, -1, SQLITE_TRANSIENT);
