...
//gcc -o getproj.o getproj.c -lsqlite3 -ljansson  -lfcgi
...
int main() {
 while (FCGI_Accept() >= 0) {
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            json_object_set_new(object, "program", json_string((const char*)sqlite3_column_text(stmt, 2)));
            json_object_set_new(object, "date", json_string((const char*)sqlite3_column_text(stmt, 3)));
            json_object_set_new(object, "time_borrow", json_string((const char*)sqlite3_column_text(stmt, 4)));
            json_object_set_new(object, "return_time", json_string((const char*)sqlite3_column_text(stmt, 5)));
            json_object_set_new(object, "item", json_string((const char*)sqlite3_column_text(stmt, 6)));
            json_object_set_new(object, "others", json_string((const char*)sqlite3_column_text(stmt, 7)));
            json_object_set_new(object, "purpose", json_string((const char*)sqlite3_column_text(stmt, 8)));
